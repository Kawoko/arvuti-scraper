-- ===========================================================================
-- Arvutitark Price Tracker - summary view for grouped, twice-daily data
-- ===========================================================================
-- Supersedes the view created in migration 0003, which keyed everything on
-- product id alone. It must run AFTER 0004, because it relies on
-- `price_history.category` and on the (id, category) composite key, neither of
-- which exists before then.
--
-- Everything here is per (product, group), so a product pinned in the `custom`
-- group keeps its own independent history and statistics.
-- ===========================================================================
...
-- The observation immediately BEFORE the latest one, per (product, group).
previous_obs as (
  select
...
comment on view public.product_price_summary is
  'Per (product, group) derived price statistics. start_price = earliest observation in our DB for that group.';
